...
import { crearSolicitud, verificarSolicitudPendiente } from '../../controllers/solicitudesController';
...
const TutoresDetailsScreen: React.FC<TutorDetailsScreenProps> = () => {
  ...
  const handleRequestTutoring = async () => {
    try {
      ...
      const solicitudExistente = await verificarSolicitudPendiente(tutorId, estudianteId);
      ...
      const fetchTutor = async () => {
        ...
        try {
          ...
          const tutorAdaptado: TutorWithMaterias = {
            ...data,
            materiasDominadas: data.materiasDominadas.map((materia) => ({ id: materia.id, materia: materia.materia })), // Extraemos solo los nombres de las materias
          };
          ...
          if (usuario) {
            const estudiante = JSON.parse(usuario);
            const solicitudExistente = await verificarSolicitudPendiente(tutorId, estudiante.id);
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <TouchableOpacity
          style={[styles.requestTutoringButton, isRequested && styles.disabledButton]}
